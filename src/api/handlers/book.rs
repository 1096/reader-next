...
const MAX_AVAILABLE_CONCURRENT_COUNT: usize = 128;
const DEFAULT_AVAILABLE_SSE_RESULT_LIMIT: usize = 120;
const MAX_AVAILABLE_SSE_RESULT_LIMIT: usize = 1000;
...
#[derive(Debug, Deserialize)]
pub struct PinChapterRequest {
    #[serde(rename = "bookUrl", alias = "url")]
    pub book_url: Option<String>,
    #[serde(rename = "chapterUrl", alias = "href")]
    pub chapter_url: Option<String>,
    #[serde(rename = "bookSourceUrl", alias = "origin")]
    pub book_source_url: Option<String>,
    #[serde(rename = "bookSource")]
...
pub async fn get_book_content(
    ...
    if let Ok(Some(pinned_content)) = state
        .book_service
        .load_pinned_chapter_content(&user_ns, &book_url, &chapter_url)
        .await
    {
        return Ok(Json(ApiResponse::ok(serde_json::Value::String(pinned_content))));
    }
...
pub async fn pin_book_chapter_content(
    State(state): State<AppState>,
    auth: AuthContext,
    Query(q): Query<PinChapterRequest>,
    body: axum::body::Bytes,
) -> Result<Json<ApiResponse<serde_json::Value>>, AppError> {
    let user_ns = state
        .user_service
    ...
    let mut req = q;
    if !body.is_empty() {
        if let Ok(v) = serde_json::from_slice::<PinChapterRequest>(&body) {
    ...
    state
        .book_service
        .save_pinned_chapter_content(&user_ns, &book_url, &chapter_url, &content)
        .await?;
...
pub async fn search_book_source_sse(
    ...
    let concurrent = effective_available_concurrent_count(q.concurrent_count);
    let book_source_group =
        q.book_source_group
            .clone()
            .and_then(|g| if g.trim().is_empty() { None } else { Some(g) });
            ...
            if let Some(res) = tasks.next().await {
                if let Ok((cur_idx, Ok(list), target_name, target_author)) = res {
                    ...
                    let progress_payload = serde_json::json!({
                        ...
                        "matched": total,
                        "lastIndex": cur_idx,
                        "hasMore": (idx as usize) < sources.len() || !tasks.is_empty(),
                    });
                    let _ = tx
                        .send(
                            Event::default()
...
pub async fn get_available_book_source_sse(
    ...
    let concurrent_count = effective_available_concurrent_count(req.concurrent_count);
    let sse_result_limit = effective_available_sse_result_limit(req.search_size, req.result_limit);
    let book_url = req.url.clone();
    ...
    tokio::spawn(async move {
        ...
        while next_index < sources.len() || !tasks.is_empty() {
            ...
            if emitted >= sse_result_limit || tasks.is_empty() {
                break;
            }
...
fn effective_available_sse_result_limit(search_size: Option<i32>, result_limit: Option<i32>) -> usize {
    search_size
        .or(result_limit)
        .unwrap_or(DEFAULT_AVAILABLE_SSE_RESULT_LIMIT as i32)
        .clamp(1, MAX_AVAILABLE_SSE_RESULT_LIMIT as i32) as usize
}
