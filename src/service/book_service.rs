...
impl BookService {
    ...
    pub async fn load_pinned_chapter_content(
        &self,
        user_ns: &str,
        book_url: &str,
        chapter_url: &str,
    ) -> Result<Option<String>, AppError> {
        let path = self.pinned_chapter_content_path(user_ns, book_url, chapter_url);
        if !path.exists() {
            return Ok(None);
        }
    ...
    pub async fn save_pinned_chapter_content(
        &self,
        user_ns: &str,
        book_url: &str,
        chapter_url: &str,
    ...
    pub async fn delete_pinned_book_content(
        &self,
        user_ns: &str,
        book_url: &str,
    ) -> Result<(), AppError> {
        let path = self.pinned_book_content_dir(user_ns, book_url);
        if path.exists() {
    ...
    fn pinned_book_content_dir(&self, user_ns: &str, book_url: &str) -> PathBuf {
        let name = md5_hex(book_url);
        self.storage_dir
            .join("data")
            .join(user_ns)
            .join("pinned_chapters")
            .join(name)
    }

    fn pinned_chapter_content_path(&self, user_ns: &str, book_url: &str, chapter_url: &str) -> PathBuf {
        let chapter_name = md5_hex(chapter_url);
        self.pinned_book_content_dir(user_ns, book_url)
            .join(format!("{}.txt", chapter_name))
    }
    ...
    async fn clear_book_related_cache(&self, user_ns: &str, book: &Book) -> Result<(), AppError> {
        if !book.book_url.is_empty() {
            ...
            let _ = self.delete_pinned_book_content(user_ns, &book.book_url).await;
            let _ = self
