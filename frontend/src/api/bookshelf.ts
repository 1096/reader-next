...
export function pinBookChapterContent(params: {
  bookUrl?: string
  chapterUrl: string
  bookSourceUrl?: string
  index?: number
}) {
  return http.post<{ bookUrl: string; chapterUrl: string; size: number }>('/pinBookChapterContent', params).then((r) => r.data)
}
...
export function setBookSource(params: {
  ...
  return http.post<Book>('/setBookSource', params, { timeout: 30000 }).then((r) => r.data)
}
