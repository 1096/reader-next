  ...
  <div class="reader-catalog" :style="{ background: theme.popup, color: theme.fontColor }">
    ...
    <div v-show="activeTab === 'chapters'" class="chapter-toolbar">
      ...
      <div class="chapter-jump-actions">
        <button
          class="jump-btn"
          :class="{ active: chapterPinEditMode }"
          title="批量固化章节"
          :disabled="store.chaptersLoading || !filteredChapters.length"
          @click="toggleChapterPinMode"
        >
    ...
    <div v-if="activeTab === 'chapters' && chapterPinEditMode" class="chapter-pin-toolbar">
      <button class="pin-action" @click="selectAllFilteredChapters">全选当前筛选</button>
      <button class="pin-action" @click="clearSelectedChapters">清空</button>
      <button
        class="pin-action primary"
        :disabled="!selectedChapterIndexes.size || pinningChapters"
        @click="pinSelectedChapters"
      >
        {{ pinningChapters ? `固化中(${pinProgress.done}/${pinProgress.total})` : `固化选中(${selectedChapterIndexes.size})` }}
      </button>
      <button class="pin-action" @click="toggleChapterPinMode">完成</button>
    </div>
    ...
    <div v-show="activeTab === 'chapters'" class="list-container" ref="listRef">
      ...
      >
        <button
          v-if="chapterPinEditMode"
          class="chapter-check"
          :class="{ checked: selectedChapterIndexes.has(chapter.index) }"
          @click.stop="toggleChapterSelection(chapter.index)"
        >
          ✓
        </button>
...
import { pinBookChapterContent } from '../../api/bookshelf'
...
const chapterPinEditMode = ref(false)
const selectedChapterIndexes = ref<Set<number>>(new Set())
const pinningChapters = ref(false)
const pinProgress = ref({ done: 0, total: 0 })
...
function toggleChapterPinMode() {
  chapterPinEditMode.value = !chapterPinEditMode.value
  if (!chapterPinEditMode.value) {
...
function toggleChapterSelection(index: number) {
  if (selectedChapterIndexes.value.has(index)) {
    selectedChapterIndexes.value.delete(index)
  } else {
    selectedChapterIndexes.value.add(index)
  }
}

function selectAllFilteredChapters() {
  selectedChapterIndexes.value = new Set(filteredChapters.value.map((item) => item.index))
}

function clearSelectedChapters() {
  selectedChapterIndexes.value.clear()
  pinProgress.value = { done: 0, total: 0 }
}

async function pinSelectedChapters() {
  if (!store.book || !selectedChapterIndexes.value.size || pinningChapters.value) return
  const chapterList = [...selectedChapterIndexes.value]
    .sort((a, b) => a - b)
    .map((index) => ({ index, chapter: store.chapters[index] }))
    .filter((item) => !!item.chapter?.url)

  pinningChapters.value = true
  pinProgress.value = { done: 0, total: chapterList.length }
  let success = 0

  try {
    for (const item of chapterList) {
      try {
        await pinBookChapterContent({
      ...
      pinProgress.value.done += 1
    }
    if (success > 0) {
      appStore.showToast(`已固化 ${success}/${chapterList.length} 章`, 'success')
    } else {
      appStore.showToast('批量固化失败，请重试', 'error')
    }
  } finally {
    pinningChapters.value = false
    clearSelectedChapters()
    chapterPinEditMode.value = false
  }
}
